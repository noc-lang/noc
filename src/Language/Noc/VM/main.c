...
void run(NocBytecode b) {
    int i = 0;
    while(i < b.opcodes.opcodes.size) {
        if(b.opcodes.opcodes.elems[i].label == RETURN) {
            if(vm.callstack.cursor > 0) {
                NocValue index = pop_stack(&vm.callstack);
                i = index.i;
            } else { exit(EXIT_SUCCESS); }
        } else if(b.opcodes.opcodes.elems[i].label == CALL_SYMBOL) {
            Sym sym_elem = b.sym.sym[b.opcodes.opcodes.elems[i].operand];
            ...
            void (*f)(NocBytecode, NocOp) = OPCODES_FUNCS[b.opcodes.opcodes.elems[i].label];
            f(b, b.opcodes.opcodes.elems[i]);
            i++;
        }
    }
}
